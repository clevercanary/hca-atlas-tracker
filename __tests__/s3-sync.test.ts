...
describe("syncFilesFromS3", () => {
  it("processes s3 data, logs warnings, and creates and validates files as appropriate", async () => {
    await doMainTest();
  });
});
...
async function doMainTest(): Promise<void> {
  ...
  expect(filesByKey.get(KEY_NO_LENGTH)).toMatchObject(EXPECTED_FILE_NO_LENGTH);
