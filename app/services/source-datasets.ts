...
export async function getAtlasDatasets(
  atlasId: string
): Promise<HCAAtlasTrackerDBSourceDatasetFile[]> {
  ...
  const queryResult = await query<HCAAtlasTrackerDBSourceDatasetFile>(
    `
      SELECT
        f.id,
        f.integrity_status,
        ...
        f.status
      FROM hat.files f
      JOIN hat.source_datasets sd ON f.source_dataset_id = sd.id
      WHERE sd.id= ANY($1)
    `,
    [sourceDatasetIds]
  );
...
export async function clearSourceDatasetInfo(
  sourceDatasetId: string,
  client?: pg.PoolClient
): Promise<void> {
  await doOrContinueTransaction(client, async (tx) => {
    ...
    await tx.query(
      "UPDATE hat.source_datasets SET sd_info = '{}'::jsonb WHERE id = $1",
      [sourceDatasetId]
    );
    await updateFieldsForComponentAtlasesHavingSourceDatasets(
      [sourceDatasetId],
